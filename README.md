...
- `--medium`: Butterfly algorithm
	...
	- We picked this algorithm because we understood that a "range-based" algorithm was the overall dominating strategy for a `√n` complexity. The only real choice was how to implement it, and this implementation is the one that made the most sense for us.

- `--complex`: Radix sort
	- An optimized O(nlogn) sorting algorithm.
	- Radix sort operates by comparing each digit of numbers one by one and assigning the numbers to groups based on their digit. For each iteration, the digits within the groups are sorted according to the previous digits. Once the final digit is reached, everything is in order. It is not the smartest way to go about it, as it will always take the same number of operations whether the stack is roughly ordered or not, but its advantage is that it thrives in sorting large quantities, being our most optimized algorithm around 1000 items.
	- As we only have two stacks to compare ten digits, the implementation of the radix logic to push_swap required us to use base two _(aka binary)_. This was possible by using two of the bitwise operators available in C: **`>>` (right shift)** and **`&` (bitwise AND)**.
